...
cv::Mat normalize_kernel(const cv::Mat &kernel, int rows, int cols) {
    ...
    {
        cv::Mat target_top_right = kernel_spectral(cv::Rect(
    ...
    {
        cv::Mat target_bottom_right = kernel_spectral(
    ...
    return kernel_spectral / cv::sum(kernel_spectral);
}
...
cv::Mat get_gaussian_kernel(double sigma, size_t size) {
    ...
    for (int col = 0; col < int(size); col++) {
        for (int row = 0; row < int(size); row++) {
            double x = 2.0 * double(col) / double(size - 1) - 1.0;
            double y = 2.0 * double(row) / double(size - 1) - 1.0;
